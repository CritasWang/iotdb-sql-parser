...
// ------------------------------------------- Table Statement ---------------------------------------------------------
createTableStatement
    : CREATE TABLE (IF NOT EXISTS)? qualifiedName '(' (columnDefinition (',' columnDefinition)*)? ')' charsetDesc? comment? (
        WITH properties
    )?
    ;
...
// ------------------------------------------- Table View Statement ---------------------------------------------------------
createViewStatement
    : CREATE (OR REPLACE)? VIEW qualifiedName '(' (viewColumnDefinition (',' viewColumnDefinition)*)? ')' comment? (
        RESTRICT
    )? (WITH properties)? AS prefixPath
    ;
...
loadFileWithAttributesClause
    : WITH '(' (loadFileWithAttributeClause ',')* loadFileWithAttributeClause? ')'
    ;
...
extractorAttributesClause
    : WITH (EXTRACTOR | SOURCE) '(' (extractorAttributeClause ',')* extractorAttributeClause? ')'
    ;
...
processorAttributesClause
    : WITH PROCESSOR '(' (processorAttributeClause ',')* processorAttributeClause? ')'
    ;
...
connectorAttributesClause
    : WITH (CONNECTOR | SINK) '(' (connectorAttributeClause ',')* connectorAttributeClause? ')'
    ;
...
alterExtractorAttributesClause
    : (MODIFY | REPLACE) (EXTRACTOR | SOURCE) '(' (extractorAttributeClause ',')* extractorAttributeClause? ')'
    ;

alterProcessorAttributesClause
    : (MODIFY | REPLACE) PROCESSOR '(' (processorAttributeClause ',')* processorAttributeClause? ')'
    ;

alterConnectorAttributesClause
    : (MODIFY | REPLACE) (CONNECTOR | SINK) '(' (connectorAttributeClause ',')* connectorAttributeClause? ')'
    ;
...
// -------------------------------------------- Show Statement ---------------------------------------------------------
showDevicesStatement
    : SHOW DEVICES FROM tableName= qualifiedName (WHERE where= booleanExpression)? limitOffsetClause
    ;
...
showQueriesStatement
    : SHOW (QUERIES | QUERY PROCESSLIST) (WHERE where= booleanExpression)? (
...
createModelStatement
    ...
    | CREATE MODEL modelId= identifier (WITH HYPERPARAMETERS '(' hparamPair (',' hparamPair)* ')')? FROM MODEL existingModelId= identifier ON DATASET '('
        targetData= string ')'
    ;
...
queryNoWith
    : queryTerm fillClause? (ORDER BY sortItem (',' sortItem)*)? limitOffsetClause
    ;
...
patternRecognition
    : aliasedRelation (
        MATCH_RECOGNIZE '(' (PARTITION BY partition+= expression ( ',' partition+= expression)*)? (
            ORDER BY sortItem (',' sortItem)*
        )? (MEASURES measureDefinition (',' measureDefinition)*)? rowsPerMatch? (AFTER MATCH skipTo)? (
            INITIAL
            | SEEK
        )? PATTERN '(' rowPattern ')' (SUBSET subsetDefinition (',' subsetDefinition)*)? DEFINE variableDefinition (
            ',' variableDefinition
