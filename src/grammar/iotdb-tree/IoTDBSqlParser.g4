...
alignedMeasurements
    : LR_BRACKET nodeNameWithoutWildcard attributeClauses (
        COMMA nodeNameWithoutWildcard attributeClauses
    )* RR_BRACKET
    ;
...
// Device Template
createSchemaTemplate
    : CREATE (SCHEMA | DEVICE) TEMPLATE templateName= identifier ALIGNED? (
...
dropTimeseriesOfSchemaTemplate
    : ((DELETE | DROP) TIMESERIES OF | DEACTIVATE) (SCHEMA | DEVICE) TEMPLATE (templateName= identifier)? FROM prefixPath (
        COMMA prefixPath
    )*
    ;
...
alterSchemaTemplate
    : ALTER (SCHEMA | DEVICE) TEMPLATE templateName= identifier ADD LR_BRACKET templateMeasurementClause (
        COMMA templateMeasurementClause
    )* RR_BRACKET
    ;
...
createContinuousQuery
    : CREATE (CONTINUOUS QUERY | CQ) cqId= identifier resampleClause? timeoutPolicyClause? BEGIN selectStatement END
    ;

resampleClause
    : RESAMPLE (EVERY everyInterval= DURATION_LITERAL)? (BOUNDARY boundaryTime= timeValue)? (
...
// ---- Get Region Id
getRegionId
    : SHOW (DATA | SCHEMA) REGIONID WHERE (
...
// ---- Get Time Slot List
getTimeSlotList
    : SHOW (TIMESLOTID | TIMEPARTITION) WHERE (
        DEVICE operator_eq device= prefixPath
...
countTimeSlotList
    : COUNT (TIMESLOTID | TIMEPARTITION) WHERE (
        DEVICE operator_eq device= prefixPath
        | REGIONID operator_eq regionId= INTEGER_LITERAL
        | DATABASE operator_eq database= prefixPath
...
extractorAttributesClause
    : WITH (EXTRACTOR | SOURCE) LR_BRACKET (extractorAttributeClause COMMA)* extractorAttributeClause? RR_BRACKET
    ;
...
connectorAttributesClause
    : WITH (CONNECTOR | SINK) LR_BRACKET (connectorAttributeClause COMMA)* connectorAttributeClause? RR_BRACKET
    ;
...
alterPipe
    : ALTER PIPE (IF EXISTS)? pipeName= identifier alterExtractorAttributesClause? alterProcessorAttributesClause? alterConnectorAttributesClause?
    ;

alterExtractorAttributesClause
    : (MODIFY | REPLACE) (EXTRACTOR | SOURCE) LR_BRACKET (extractorAttributeClause COMMA)* extractorAttributeClause? RR_BRACKET
    ;

alterProcessorAttributesClause
    : (MODIFY | REPLACE) PROCESSOR LR_BRACKET (processorAttributeClause COMMA)* processorAttributeClause? RR_BRACKET
    ;

alterConnectorAttributesClause
    : (MODIFY | REPLACE) (CONNECTOR | SINK) LR_BRACKET (connectorAttributeClause COMMA)* connectorAttributeClause? RR_BRACKET
    ;
...
showPipes
    : SHOW (
        (PIPE pipeName= identifier)
        | PIPES ( WHERE (CONNECTOR | SINK) USED BY pipeName= identifier)?
    )
    ;
...
createModel
    ...
    )? FROM MODEL existingModelId= identifier ON DATASET LR_BRACKET trainingData RR_BRACKET
    ;
...
// Table view
createTableView
    : CREATE (OR REPLACE)? VIEW qualifiedName LR_BRACKET (
        viewColumnDefinition (COMMA viewColumnDefinition)*
    )? RR_BRACKET comment? (RESTRICT)? (WITH properties)? AS prefixPath
    ;
...
// Select Statement
selectStatement
    : selectClause intoClause? fromClause whereClause? groupByClause? havingClause? orderByClause? fillClause? paginationClause? alignByClause?
    | selectClause intoClause? fromClause whereClause? groupByClause? havingClause? fillClause? orderByClause? paginationClause? alignByClause?
    ;
...
showQueries
    : SHOW (QUERIES | QUERY PROCESSLIST) whereClause? orderByClause? rowPaginationClause?
    ;
...
loadFileWithAttributeClauses
    : WITH LR_BRACKET (loadFileWithAttributeClause COMMA)* loadFileWithAttributeClause? RR_BRACKET
    ;
...
expression
    ...
    | unaryBeforeRegularOrLikeExpression= expression operator_not? LIKE pattern= STRING_LITERAL (
        ESCAPE escapeSet= STRING_LITERAL
    )?
    ...
    | unaryBeforeInExpression= expression operator_not? (operator_in | operator_contains) LR_BRACKET constant (
        COMMA constant
    )* RR_BRACKET
...
attributeClauses
    : aliasNodeName? WITH attributeKey operator_eq dataType= attributeValue (COMMA? attributePair)* tagClause? attributeClause?
    // Simplified version (supported since v0.13)
    | aliasNodeName? WITH? (attributeKey operator_eq)? dataType= attributeValue attributePair* tagClause? attributeClause?
    ;
...
subStringExpression
    : LR_BRACKET input= expression COMMA startPosition= signedIntegerLiteral (
        COMMA length= signedIntegerLiteral
    )? RR_BRACKET
